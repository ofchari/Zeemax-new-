...
class PdfPoView extends StatefulWidget {
  final String name;
  final String type; // Add type to handle different types of POs

  const PdfPoView({super.key, required this.name, required this.type});

  @override
  State<PdfPoView> createState() => _PdfPoViewState();
}

class _PdfPoViewState extends State<PdfPoView> {
  late double height;
  late double width;
  String? pdfFilePath;
  bool isLoading = true;
  ...
  /// Map the type to the correct API format.
  String _getPdfApiUrl(String name, String type) {
    switch (type.toLowerCase()) {
      case 'product':
        return "https://zeemax.regenterp.com/api/method/frappe.utils.print_format.download_pdf?doctype=PO&name=$name&format=PO&no_letterhead=1&letterhead=No%20Letterhead&settings=%7B%7D&_lang=en";
      case 'yarn':
        return "https://zeemax.regenterp.com/api/method/frappe.utils.print_format.download_pdf?doctype=PO&name=$name&format=PO&no_letterhead=1&letterhead=No%20Letterhead&settings=%7B%7D&_lang=en";
      case 'fabric':
        return "https://zeemax.regenterp.com/api/method/frappe.utils.print_format.download_pdf?doctype=PO&name=$name&format=PO&no_letterhead=1&letterhead=No%20Letterhead&settings=%7B%7D&_lang=en";
  ...
  Future<void> _fetchAndLoadPdf() async {
    try {
      String apiUrl = _getPdfApiUrl(widget.name, widget.type);
  ...
  Future<void> workflow_update() async {
    ...
    final url = Uri.parse(
        'https://zeemax.regenterp.com/api/resource/PO/${widget.name}');
            ...
            child: GestureDetector(
              onTap: () {
                ...
                workflow_update().then((_) {
                  ...
                  Get.offAll(() => POScreen(
                      updateWorkflowState:
                          actionTake)); // Pass the updated workflow state to the OrderForm page
                }).catchError((e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Subhead(text: "PDF Po", color: Colors.white),
